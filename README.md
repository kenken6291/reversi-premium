# Reversi Premium (オセロ/リバーシゲーム)

高品質なビジュアルデザインと3段階のAI、さらにリアルタイム通信対戦を搭載した、ブラウザで動作するプレミアムなオセロ（リバーシ）ゲームです。
...
- **3段階のAI難易度**:
  - **簡単**: 合法手（置けるマス）の中からランダムに配置。
  - **普通**: 1手評価（パターンスコアと位置の評価）に基づき、自分の石を増やし、有利な位置を取るAI。
  - **難しい**: アルファベータ法による先読み（深さ4）と評価マップを用いて、相手の対応も考慮しつつ最善手を選択する本格AI。
- **プレミアムデザイン**:
...
- **充実の便利機能**:
  - 1手戻す機能（Undo / AI戦ではプレイヤー・AI of 2手分戻ります）。
...
4. 説明（Description）に「HTML5 Reversi Game with AI & Firebase」などと入力します。
...
git remote add origin https://github.com/ご自身のユーザー名/reversi-premium.git
...
6. 数分待つと、画面上部に公開されたURL（例: `https://ユーザー名.github.io/reversi-premium/`）が表示されます。このURLを友達に共有すれば、スマホや他のPCからオンライン対戦などを一緒に遊ぶことができます！
